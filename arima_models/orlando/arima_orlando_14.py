...
# overview charts, per-placekey plots, and Top-10 best/worst exports.
...
def parse_args():
    ...
    ap.add_argument("--input", default="ts_daily_panel_small/orlando_daily_panel.parquet",
                    help="Input daily panel (CSV or Parquet)")
    ap.add_argument("--city", default="Orlando", help="City name for outputs")
    ap.add_argument("--target", default="d14", choices=["d14","d21"],
                    help="Forecast target: day 14 or 21")
    ap.add_argument("--outdir", default="arima_forecasts_small", help="Output dir")
    ...
    ap.add_argument("--plot-topk", type=int, default=0,
                    help="Also plot Top-K best (lowest error). 0=skip")
    ap.add_argument("--plot-worstk", type=int, default=0,
...
def plot_single_placekey(row, outdir: Path):
    """Per-placekey figure: blue line (days 1-13), markers for actual/pred D14, optional 80% CI, Ian dashed."""

    name = str(row.get("location_name","")); city = str(row.get("city",""))
    pk = str(row.get("placekey","")); lat = row.get("latitude", np.nan); lon = row.get("longitude", np.nan)
    ...
    dates_train = pd.date_range(start_date, periods=13, freq='D')
    ...
    # try to compute simple 80% CI via refit (fast)
    ci_low = ci_high = None
    try:
        ...
        m = ARIMA(y, order=order, enforce_stationarity=False,
                  enforce_invertibility=False, concentrate_scale=True)
        fit = m.fit(method_kwargs={"warn_convergence": False})
        ci = fit.get_forecast(steps=1).conf_int(alpha=0.2)
        ci_low, ci_high = float(ci.iloc[0,0]), float(ci.iloc[0,1])
    except Exception:
        pass

    fig, ax = plt.subplots(figsize=(12,7))
    ax.plot(dates_train, prev, "o-", linewidth=2, markersize=6, label="Observed (Days 113)")
    if ci_low is not None and ci_high is not None:
        ax.fill_between([d14_date - pd.Timedelta(hours=12), d14_date + pd.Timedelta(hours=12)],
                        [ci_low, ci_low], [ci_high, ci_high], alpha=0.2, label="80% CI")
    if np.isfinite(y_true):
        ax.plot(d14_date, y_true, "o", markersize=10, label=f"Actual D14 = {y_true:.1f}")
    if np.isfinite(y_pred):
        ax.plot(d14_date, y_pred, "s", markersize=10, label=f"ARIMA D14 = {y_pred:.1f}")

    if dates_train.min() <= IAN_LANDFALL <= d14_date:
        ax.axvline(IAN_LANDFALL, linestyle="--", alpha=0.7, label="Ian landfall")
    ...
    ax.set_xlabel("Date"); ax.set_ylabel("Visits"); ax.grid(True, alpha=0.3); ax.legend()

    safe = "".join(c if c.isalnum() or c in " _-@" else "_" for c in name)[:60]
    outpath = outdir / f"{safe}_{pk}.png"
...
def main():
    ...
    # Top-10 best/worst CSVs + optional PNGs
    valid = results_df.dropna(subset=['absolute_error']).copy()
    if len(valid) > 0:
    ...
    if args.plot_each > 0 and len(results_df) > 0:
        ...
        print(f"Making {len(idx)} per-placekey plots (random subset)...")
        for i, ridx in enumerate(idx, 1):
            p = plot_single_placekey(results_df.loc[ridx], outdir)
