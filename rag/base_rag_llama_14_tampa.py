#!/usr/bin/env python3
"""
RAG-Enhanced Hurricane Forecasting using base model + evacuation knowledge base
"""
...
def parse_args():
    ...
    p.add_argument("--max-new-tokens", type=int, default=100, help="Maximum tokens to generate")
...
def parse_prediction(text):
    t = (text or "").strip()
    # Look for PREDICTION: format first
    ...
    if m:
        ...
        conf = max(0.0, min(conf, 100.0)) / 100.0  # normalize to 0-1
        return pred, conf
...
def main():
    ...
    print("=== RAG-Enhanced Hurricane Forecasting (Base Model) ===")
        ...
        try:
            ...
            if pred is not None:
                ...
                results.append({
                    ...
                    "generated_text": generated_text[:500],  # Keep more text for analysis
                })
            else:
            ...
            if (idx + 1) % 50 == 0:  # More frequent updates
                success_rate = ((idx + 1 - failed_predictions) / (idx + 1)) * 100
                print(f"Processed {idx + 1}/{len(test_records)} samples (Success rate: {success_rate:.1f}%)")
    ...
    if y_true_list and y_pred_list:
        ...
        print("\n=== RAG-Enhanced Results (Base Model) ===")
        print(f"Samples processed: {len(results)}")
        print(f"Failed predictions: {failed_predictions}")
        print(f"Success rate: {(len(results) / (len(results) + failed_predictions)) * 100:.1f}%")
        ...
        results_df.to_csv(output_dir / "rag_base_model_results.csv", index=False)

        # Save summary
        summary = {
            "model_type": "RAG_base_model",
            "base_model": args.base_model,
            "knowledge_base": args.knowledge_base,
            "total_samples": len(test_records),
            "successful_predictions": len(results),
            "failed_predictions": failed_predictions,
            "success_rate": (len(results) / len(test_records)) * 100,
        ...
        with open(output_dir / "rag_base_model_summary.json", "w") as f:
            json.dump(summary, f, indent=2)

        print(f"\nResults saved to: {output_dir}")
    else:
        print("[ERROR] No successful predictions were made. Check your model and data.")
