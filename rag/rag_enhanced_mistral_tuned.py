#!/usr/bin/env python3
"""
RAG-Enhanced Hurricane Forecasting using fine-tuned Mistral LoRA + evacuation knowledge base

This script combines:
1. Fine-tuned Mistral 7B LoRA adapter for forecasting
...
Usage:
    python rag_enhanced_mistral_lora.py \
        --model-dir ./mistral-hurricane-lora \
...
Output Structure:
    rag_results/
    └── rag_mistral_lora_results_Tampa/
        ├── csv_outputs/
        │   └── rag_predictions.csv
        ├── reports/
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="RAG-Enhanced Hurricane Forecasting with Fine-tuned Mistral LoRA",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Basic usage with fine-tuned LoRA
    python rag_enhanced_mistral_lora.py \\
        --model-dir ./mistral-hurricane-lora \\
        --knowledge-base evacuation_kb.json \\
    ...
    p.add_argument("--model-dir", required=True,
                   help="Local LoRA adapter directory (contains adapter_model.safetensors)")
...
# -------------------- Model Setup --------------------
def setup_lora_model_and_pipeline(base_model_name, lora_adapter_dir, use_quantization=False, 
                                  local_files_only=False, temperature=0.1, max_new_tokens=32):
    """
    Setup fine-tuned Mistral LoRA model and create generation pipeline
    
    Args:
        base_model_name: Base model identifier or path
        lora_adapter_dir: Path to LoRA adapter directory
    ...
    # Verify LoRA adapter directory exists
    adapter_path = Path(lora_adapter_dir)
    if not adapter_path.exists():
        sys.exit(f"[ERROR] LoRA adapter directory not found: {adapter_path}")
    ...
    tokenizer = AutoTokenizer.from_pretrained(base_model_name, **model_kwargs)
    ...
    # Setup base model loading configuration
    ...
    base_model = AutoModelForCausalLM.from_pretrained(base_model_name, **base_model_kwargs)
    ...
    model = PeftModel.from_pretrained(
        base_model,
        str(adapter_path),
        local_files_only=True  # LoRA adapter is local
    )
    ...
    print("LoRA model and pipeline setup complete")
    return pipe

# -------------------- Evaluation --------------------
def evaluate_rag_lora_model(pipe, evac_kb, test_records, city_name, max_samples=None):
    """
    Evaluate RAG-enhanced LoRA model on test data
    
    Args:
        pipe: Text generation pipeline with LoRA adapter
...
def main():
    ...
    adapter_name = Path(args.model_dir).name
    results_dir = output_dir / f"rag_mistral_lora_results_{args.city}"
    ...
    print("=" * 80)
    print("RAG-Enhanced Hurricane Forecasting with Fine-tuned Mistral LoRA")
    print("=" * 80)
    ...
    try:
        ...
        # Setup LoRA model and pipeline
        print(f"\n{'-'*50}")
        print("Setting Up Fine-tuned LoRA Model and Pipeline")
        print(f"{'-'*50}")
        pipe = setup_lora_model_and_pipeline(
            args.base_model,
            args.model_dir,
        ...
        # Run evaluation
        print(f"\n{'-'*50}")
        print("Running RAG-Enhanced LoRA Evaluation")
        print(f"{'-'*50}")
        results_df, metrics = evaluate_rag_lora_model(
        ...
        execution_summary = {
            "script_info": {
                "script_name": "rag_enhanced_mistral_lora.py",
            ...
            "configuration": {
                ...
                "lora_adapter": str(Path(args.model_dir).name),
