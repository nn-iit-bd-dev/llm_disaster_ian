...
def parse_args():
    ...
    # Keep backward compatibility - if someone uses old --input arg, treat as train data
    p.add_argument("--input", help="Legacy: Training JSONL (use --train-input instead)")
    p.add_argument("--test-split", type=float, default=0.2, help="Legacy: ignored when using separate files")
    return p.parse_args()
...
def load_and_prepare_data(train_jsonl_path, test_jsonl_path, limit=None, legacy_input=None, test_split=0.2):
    """Load pre-split training and test data, or handle legacy single file"""
    
    # Handle legacy mode for backward compatibility
    if legacy_input and not train_jsonl_path:
        print("Using legacy mode: splitting single input file")
        rows = []
        with open(legacy_input, "r", encoding="ascii", errors="ignore") as f:
            for line in f:
                if line.strip():
                    rows.append(json.loads(line))
        ...
        df = pd.DataFrame(rows)
        if df.empty:
            raise ValueError("No records in prepared JSONL.")
        ...
        train_texts, test_texts = train_test_split(
            training_texts, test_size=test_split, random_state=42
        )
        
        # Also split the DataFrame for evaluation
        train_indices, test_indices = train_test_split(
            range(len(df)), test_size=test_split, random_state=42
        )
...
def parse_prediction(text):
    ...
    # Fallback 1: Look for any number after "PREDICTION"
    ...
    # Fallback 2: Pattern like "2.0 CONFIDENCE: 90%" (missing PREDICTION:)
    direct_match = re.search(r'^([0-9]*\.?[0-9]+)\s+CONFIDENCE:\s*([0-9]*\.?[0-9]+)', text, re.I)
    if direct_match:
        pred = max(0.0, min(float(direct_match.group(1)), 10000.0))
        conf = float(direct_match.group(2))
        return pred, conf/100.0
    
    # Fallback 3: Just number + confidence pattern
    number_conf = re.search(r'([0-9]*\.?[0-9]+).*CONFIDENCE:\s*([0-9]*\.?[0-9]+)', text, re.I)
    if number_conf:
        pred = max(0.0, min(float(number_conf.group(1)), 10000.0))
        conf = float(number_conf.group(2))
        return pred, conf/100.0
    
    # Fallback 4: Extract first number in text
    nums = re.findall(r'([0-9]+\.?[0-9]*)', text)
    if nums:
        pred = max(0.0, min(float(nums[0]), 10000.0))
        return pred, 0.3
    
    # Fallback 5: Look for any floating point number
...
def evaluate_model(model, tokenizer, test_df, outdir):
    ...
    pipe = pipeline(
        ...
        device_map="auto",
        do_sample=False,
        max_new_tokens=30,  # Increased for better completion
    ...
    for idx, row in test_df.iterrows():
        ...
        try:
            ...
            if pred is not None:
                ...
                # Calculate basic individual metrics without separate function
                ...
                if idx < 5:  # Print first few failures for debugging
    ...
    if not results_df.empty and len(y_true_list) > 0:
        # Calculate overall metrics inline
        ...
        # Save results
        results_df.to_csv(outdir / "finetuned_predictions.csv", index=False, encoding="ascii", errors="ignore")
        
        # Save detailed metrics by category
        if "top_category" in results_df.columns and len(results_df) > 0:
            try:
                ...
                category_metrics.to_csv(outdir / "category_metrics.csv", index=False, encoding="ascii", errors="ignore")
            except Exception as e:
        ...
        if not results_df.empty:
            results_df.to_csv(outdir / "failed_predictions_debug.csv", index=False, encoding="ascii", errors="ignore")
        return {}
...
def main():
    ...
    if not args.train_input and not args.input:
        print("Error: Either --train-input or --input (legacy) must be provided")
        sys.exit(1)
    
    if args.train_input and not args.test_input:
        print("Error: When using --train-input, --test-input must also be provided")
        sys.exit(1)
    ...
    if args.train_input and args.test_input:
        ...
        print(f"Using legacy single file mode: {args.input}")
        train_texts, test_texts, test_df = load_and_prepare_data(
            None, None, args.limit, args.input, args.test_split
        )
    ...
    with open(outdir / "training_summary.json", "w", encoding="ascii", errors="ignore") as f:
